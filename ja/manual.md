...
正式名をロゴと合わせて検討中です。
以前の仮称`XVRF`はプロジェクト名としてしばらく残ります。
以下が現在の候補です。
...
> * Virtueldと選択したキャラクターモデルとの相性に何か問題がある場合には、`Loading...`の途中または終了後に、通常と異なる挙動が起きる可能性があります。
...
> もし、認可コードが入力できない場合、仕様変更などの可能性があるので、当方(Virtueld側)にお問い合わせください。
...
2. ページ内の「♡」を押してみましょう。
> * Virtueldでは、利用条件が`暴力表現OK`のキャラクターに限ります。
...
> * Virtueldでは、`VRoid Hub`にあるあなたのキャラクターの場合、利用条件によらず遊ぶことができます。
> * もし、同梱または標準的なものと比較して動作が良くない場合、以下を参考にしていただければ幸いです。
> ☛ [Virtueld Model Definitions / Virtueldで良く動くVRMのために](https://scrapbox.io/XVRF/XVRF_Model_Definitions_%2F_XVRF%E3%81%A7%E8%89%AF%E3%81%8F%E5%8B%95%E3%81%8FVRM%E3%81%AE%E3%81%9F%E3%82%81%E3%81%AB)
...
* `VRoid Hub`にあるキャラクターモデルに個別で、**Virtueldとしてのキャラクターモデルの特性**をユーザーが設定して、これをオンラインで共有する機能です。
  * VRMや`VRoid Hub`キャラクターモデルには含まれていない要素に関して、**キャラクターにVirtueldのための個性**を与えることができます。
...
> * この共有について、Virtueld側や他者による**書き換え**/**消失**/**再利用**などについて、保証はないものとします。
> * 強い保護が必要になった場合、Virtueld仕様の追加を検討します。
...
* `VirtueldのDiscord関連テスター`応募方法を以下にて説明しています。
...
7. `送信`を押してください。
8. 差し支えなければ、X (Twitter)などを通じて、フォームを送信したことをお知らせいただければ幸いです。

> * お手数をおかけします🙇

## 📧情報共有 / 連絡先

* 推奨X (Twitter)ハッシュタグ（仮）: `#Virtueld`
* ☛ [xelfia@twitter.com](https://twitter.com/xelfia)
* [ライセンス表示](../LICENSE.md)

> `リポスト`/`いいね`してくれる皆さま、まことにありがとうございます🙇
