...
これまでのところプロジェクト名として「XVRF」（仮称）としてきました。正式名をロゴなどを合わせて検討中です。
以下が現在の候補です。しばらく呼称を併用しますので簡単に触れておきます。
...
* `Discord`に関する機能をテスター限定で実験/テスト中です。
> `0.3.21現在`: `Discord`に関する機能は主に、`🏠Lobby`にある`🔎👤🆚👤`ボタンから開始できます。
...
* 基本的には🎮ゲームパッド/🖱マウス/キーボード⌨に対応しています。機能の増減後の調整不足などにより操作できない部分を発見した場合、別の入力デバイスでの操作をお試しください。
