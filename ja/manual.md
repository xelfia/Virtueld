...
* 各画面にて、`[F12]`キーを押すと、`📷Screenshot`画面が開きます。
...
|`Back`|**前の画面**に戻って続行します。|
|||
|`Capture`|再び**前の画面**でスクリーンショットを撮影します。`📷Screenshot`画面で`[F12]`キーを押した場合も同様です。|
...
1. 実行したWindowsに、以下のような形式のフォルダ階層内のファイルに、Unityプレイヤーによる最後に起動したXVRFのログが残っているもとの思います。
このテキスト（`.log`形式）のファイルを探してみてください。
...
5. フォームの`Log`にペースト（張り付け）してください。

> * ログが長く、入りきらないようでしたら、先頭を優先して入る範囲までを張り付けてください。
